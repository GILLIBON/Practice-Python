...
"""Действия с инвентарём
   ...
   "Стоп" - останавливает цикл и завершает работу программы   
"""
item = str()
"""Имя предмета, добавляемого в инвентарь"""
weight = float()
"""Вес предмета, добавляемого в инвентарь"""
...
while action != ('Стоп'):
  ...
  if action == '+':
    ...
    item = (str(input('Введите название предмета: ')))
    isitem = inventory.get(item, None) # Определяет наличие указанного предмета в инвентаре перед его удалением
    if isitem is None:
